...
void CCoinShopWindow::OnBuyCredits(UIEventArgs eArgs)
{
	if (MESSAGEBOX->GetType())
		ShellExecute(0, 0, "https://pristontale.eu/login.php", 0, 0, SW_SHOW);
...
void CCoinShopWindow::OnItemNotAllowedSwapper(UIEventArgs eArgs)
{
	...
	if (pWindowItem)
	{
		...
		if (pItemBox)
		{
			...
			if (pcItemData)
			{
				if (pcItemData->sItem.iItemSpecialType == 0)
...
void CCoinShopWindow::OnPutItemPerfectize(UIEventArgs eArgs)
{
	...
	if (pWindowItem)
	{
		...
		if (pItemBox && pItemBox->GetItems().size() > 0 && pText)
		{
			...
			// Log to Server
			ITEMHANDLER->ProcessItemSlotAction(ESlotTypeHandle::ITEMSLOTHANDLE_Put, ESlotTypeHandleWhere::ITEMSLOTHANDLEWHERE_Perfectize, pcItemData->sItem.sItemID.ToInt(), pcItemData->sItem.iChk1, pcItemData->sItem.iChk2);
		}
	}
}
...
void CCoinShopWindow::OnPutItemSwapper(UIEventArgs eArgs)
{
	...
	if (pWindowItem)
	{
		...
		if (pItemBox && pItemBox->GetItems().size() > 0 && pText)
		{
			...
			//Log to Server
			ITEMHANDLER->ProcessItemSlotAction(ESlotTypeHandle::ITEMSLOTHANDLE_Put, ESlotTypeHandleWhere::ITEMSLOTHANDLEWHERE_Swap, pcItemData->sItem.sItemID.ToInt(), pcItemData->sItem.iChk1, pcItemData->sItem.iChk2);
		}
	}
}
...
void CCoinShopWindow::SetItemStatus(struct DefinitionItem* psItemDefinition, char* pszName, char* pszDescription)
{
	...
	if (psItemDefinition->RunSpeedMax)
	{
		AddString(0, "Speed", D3DCOLOR_ARGB(255, 236, 236, 236), FALSE);
		AddString(1, FormatString("%.1f", psItemDefinition->RunSpeedMax));
	}
...
BOOL CCoinShopWindow::OnMouseClick(class CMouse* pcMouse)
{
	...
	{
		if (pWindowItem->OnMouseClick(pcMouse, pWindow1->GetX(), pWindow1->GetY(), DX::Graphic::GetWidthBackBuffer(), DX::Graphic::GetHeightBackBuffer(), 0, 0))
			return TRUE;
	}
	...
	if (pWindowItem)
	{
		...
		case ITEMCOIN_ItemPerfect:
			...
			if (pItemBox && pItemBox->GetItems().size() > 0)
			{
				...
				// Check Space
				if (!CHARACTERGAME->CheckInventorySpace(pItemBox->GetItems()[0]))
				{
					TITLEBOX("Not enough space in inventory");
					return;
				}
...
void CCoinShopWindow::OnCheckShowHairCrown(int iType, UIEventArgs eArgs)
{
	UI::Window_ptr pWindowItem = pWindow1->GetElement<UI::Window>(WINDOWID_ItemData);
	UI::Window_ptr pWindowPreview = pWindowItem->GetElement<UI::Window>(WINDOWID_ItemPreview);
	if (pWindowPreview)
	{
		UI::CheckBox_ptr pCheckBoxChangePremium = pWindowPreview->GetElement<UI::CheckBox>(CHECKBOX_ShowHairCrown);
		UI::UnitMesh_ptr pHeadPreview = pWindowPreview->GetElement<UI::UnitMesh>(UNITMESHID_HeadPreview);
		if (pCheckBoxChangePremium->IsChecked())
		{
			pHeadPreview->SetHeadModel(ITEMHANDLER->GetHeadModel(GetItemDefinition()->sItem.sItemID.ToInt(),
				UNITDATA->sCharacterData.iRank > 0 ? UNITDATA->sCharacterData.iRank : 1));
		}
		else
		{
			pHeadPreview->SetHeadModel(ITEMHANDLER->GetHeadModel(GetItemDefinition()->sItem.sItemID.ToInt(), 0));
		}
	}
}
...
void CCoinShopWindow::DeleteAgingRecoveryItem(int iID)
{
	...
	if (pList)
	{
		...
		if (iCount > 0 && pList->GetWindowList().size() > 0)
			pList->AddAxis(0, -((28 + 8) * iCount));
	}
}
...
void CCoinShopWindow::BuildWindow1()
{
	...
	pWindow1->SetImage(UI::ImageLoader::LoadImage("game\\images\\coinshop\\background.png"));
		...
		for (std::vector<CCoinShopItem*>::iterator it = vItems.begin(); it != vItems.end(); it++)
		{
			...
			if (pcItem)
			{
				...
				if (pItemImage)
				{
					...
					{
						// Coin Image
						UI::ImageBox_ptr pCoinImageBox = std::make_shared<UI::ImageBox>(Rectangle2D(262, 13, 21, 13));
						pCoinImageBox->SetImage(UI::ImageLoader::LoadImage("game\\images\\coinshop\\coin.bmp"));
						pWindowItem->AddElement(pCoinImageBox);

						// Price
						UI::Text_ptr pItemPrice = std::make_shared<UI::Text>(Rectangle2D(286, 13, 64, 18));
...
{
	// Set Coin
	SetCoinAmount(ACCOUNTHANDLER->GetCoinAmount());
	...
	if (pcItem)
	{
		...
		if (pListItems)
		{
			...
			if (pItemImage)
			{
				...
				//pButtonBuy->SetMouseEvent( EMouseEvent::ClickDownL );
				pWindowItem->AddElement(pButtonBuy);
...
void CCoinShopWindow::BuildSwapper(BOOL bSpec)
{
	...
	if (pListItems)
	{
		...
		if (pWindowItem)
		{
			...
			pItemBox->AddRule(UI::ItemBox::ERule::RULE_Disallow, UI::ItemBox::ETypeRule::TYPERULE_ItemID, ITEMID_GreedyGauntlets);
...
void CCoinShopWindow::BuildAgingRecovery(struct AgingRecoveryDataHandle* psaData, int iCount, int iPrice)
{
	...
	if (pWindowItem)
	{
		...
		for (int i = 0; i < iCount; i++)
		{
			...
			pButtonBuy->SetEvent(UI::Event::Build(std::bind(&CCoinShopWindow::OnAgingRecoveryItemButtonClick, this, ps->iID, iPrice, std::placeholders::_1)));
...
void CCoinShopWindow::BuildPreviewCostume(UI::Window_ptr pcWindowItem)
{
	if (pcWindowItem)
	{
		...
		UI::ImageBox_ptr pImagePreview = std::make_shared<UI::ImageBox>(Rectangle2D(0, -22, 150, 190));
		...
		UI::UnitMesh_ptr pUnitMeshPreview = std::make_shared<UI::UnitMesh>(Rectangle2D(1, 20, 130, 230));
		...
		pUnitMeshPreview->SetCameraPosition(Point3D(pUnitMeshPreview->GetCameraPosition().iX, pUnitMeshPreview->GetCameraPosition().iY - 4500, pUnitMeshPreview->GetCameraPosition().iZ + 30000));
...
void CCoinShopWindow::BuildPreviewHead(UI::Window_ptr pcWindowItem)
{
	if (pcWindowItem)
	{
		...
		int iY = 62;
		UI::CheckBox_ptr pCheckBoxHairCrown = std::make_shared<UI::CheckBox>(Rectangle2D(1, iY, 13, 13));
		pCheckBoxHairCrown->SetFont("Arial", 16, 6, FALSE, FALSE, -1);
		pCheckBoxHairCrown->SetText("Show Hair Crown");
		pCheckBoxHairCrown->SetID(CHECKBOX_ShowHairCrown);
		pCheckBoxHairCrown->SetImage(UI::ImageLoader::LoadImage("game\\images\\UI\\checkbox.png"), UI::ImageLoader::LoadImage("game\\images\\UI\\checkbox_.png"));
		pCheckBoxHairCrown->SetEvent(UI::Event::Build(std::bind(&CCoinShopWindow::OnCheckShowHairCrown, this, CHECKBOX_ShowHairCrown, std::placeholders::_1)));
		pCheckBoxHairCrown->SetCheck(FALSE);

		//Shaman and Assassin doesn't have hair crown model files for hair style D to M
		if (((UNITDATA->sCharacterData.iClass == ECharacterClass::CHARACTERCLASS_Shaman ||
			UNITDATA->sCharacterData.iClass == ECharacterClass::CHARACTERCLASS_Assassin) &&
			GetItemDefinition()->sItem.sItemID.ToInt() > EItemID::ITEMID_HairPotionC)
			||
			GetItemDefinition()->sItem.sItemID.ToInt() > EItemID::ITEMID_HairPotionM)
		{
			pCheckBoxHairCrown->SetEnabled(FALSE);
		}
		else
		{
			pCheckBoxHairCrown->SetEnabled(TRUE);
		}

		pWindowPreview->AddElement(pCheckBoxHairCrown);

		iY += 20;

		UI::ImageBox_ptr pImagePreview = std::make_shared<UI::ImageBox>(Rectangle2D(0, iY, 102, 102));
		pImagePreview->SetImage(UI::ImageLoader::LoadImage("game\\images\\coinshop\\previewhair.png"));
		pWindowPreview->AddElement(pImagePreview);

		iY += 2;
		UI::UnitMesh_ptr pUnitMeshPreview = std::make_shared<UI::UnitMesh>(Rectangle2D(1, iY, 100, 99));
		pUnitMeshPreview->SetID(UNITMESHID_HeadPreview);
		pUnitMeshPreview->SetRenderBody(FALSE);
		pUnitMeshPreview->PrepareUnitData(UNITDATA);
		pUnitMeshPreview->SetHeadModel(ITEMHANDLER->GetHeadModel(GetItemDefinition()->sItem.sItemID.ToInt(), 0)); //use rank 0 here so players can see hair properly
		...
		iY += 108;
		UI::Button_ptr pRotateRightButton = std::make_shared<UI::Button>(Rectangle2D(18, iY, 17, 18));
		pRotateRightButton->SetCanHold(TRUE);
		pRotateRightButton->SetActiveImage(UI::ImageLoader::LoadActiveImage("game\\images\\coinshop\\btnrotateright.png", "game\\images\\coinshop\\btnrotateright_.png"));
		pRotateRightButton->SetEvent(UI::Event::Build(std::bind(&CCoinShopWindow::OnRotateUnitMesh, this, 1)));
		pWindowPreview->AddElement(pRotateRightButton);

		UI::Button_ptr pRotateLeftButton = std::make_shared<UI::Button>(Rectangle2D(67, iY, 17, 18));
		pRotateLeftButton->SetCanHold(TRUE);
		pRotateLeftButton->SetActiveImage(UI::ImageLoader::LoadActiveImage("game\\images\\coinshop\\btnrotateleft.png", "game\\images\\coinshop\\btnrotateleft_.png"));
		pRotateLeftButton->SetEvent(UI::Event::Build(std::bind(&CCoinShopWindow::OnRotateUnitMesh, this, 2)));
		pWindowPreview->AddElement(pRotateLeftButton);
...
void CCoinShopWindow::SetCoinAmount(int iValue)
{
	auto pText = pWindow1->GetElement<UI::Text>(TEXTID_Credits);
	pText->SetTextEx("Welcome %s!\nYou have %d Credits", (char*)*(UINT*)CHAR_DATA_PTR, iValue);
}
...
void CCoinShopWindow::Render()
{
	...
	if (pWindow1->IsOpen())
	{
		if (WINDOW_ISOPEN_PERSONALSHOP_OTHERS || WINDOW_ISOPEN_NPC_SHOP || WINDOW_ISOPEN_WAREHOUSE)
		{
			Close();
