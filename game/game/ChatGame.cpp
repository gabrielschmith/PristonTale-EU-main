...
{ //0 = not send, 1 = ok...
	...
	if (ChatCommand("/toggle_message_box_display", pszBuff))
	{
	...
	//Stability Commands
	if (ChatCommand("/gold_real", pszBuff))
	{
	...
	//Quest Commands
	if (ChatCommand("/cancel_quest", pszBuff) || ChatCommand("/cancel_quests", pszBuff) || ChatCommand("/cancel_active_quest", pszBuff)
		|| ChatCommand("/cancel_active_quests", pszBuff))
	{
		QUESTGAMEHANDLER->CancelQuests();
	}

	//Common Commands
	else if (ChatCommand("/reset_head_old", pszBuff) || ChatCommand("/reset_head_old", pszBuff))
	{
	...
	else if (ChatCommand("/reset_head", pszBuff) ||
		ChatCommand("/reset_hair_crown", pszBuff) ||
		ChatCommand("/reset_head", pszBuff))
	{
		if (CHARACTERGAME->IsStageVillage())
		{
	...
	else if (ChatCommand("/remove_hair_crown", pszBuff))
	{
		if (CHARACTERGAME->IsStageVillage())
		{
	...
	if (GM_MODE)
	{
		if (ChatCommand("/hide_me", pszBuff))
		{
		...
		else if (ChatCommand("/hide_me", pszBuff))
		{
			UNITDATA->bVisible = UNITDATA->bVisible ? FALSE : TRUE;
		}
		else if (ChatCommand("/clear_units", pszBuff))
		{
			int iClear = 0;
			// Set Pointer
		...
		if (ChatCommand("/set_fsp", pszBuff))
		{
		...
		if (ChatCommand("/set_no_delay", pszBuff))
		{
			for (int i = 0; i < 20; i++)
			{
		...
		if (ChatCommand("/input_box", pszBuff))
		{
		...
		else if (ChatCommand("/rp", pszBuff) || ChatCommand("/reload_particles", pszBuff))
		{
			SKILLMANAGERHANDLER->LoadParticles();
			CALL(0x00574740);
			AddChatBoxTextEx(CHATCOLOR_Error, "> All Particles reloaded!");
		}
		else if (ChatCommand("/hide_all", pszBuff))
		{
		...
		else if (ChatCommand("/get_range", pszBuff))
		{
		...
		else if (ChatCommand("/get_distances", pszBuff))
		{
		...
		else if (ChatCommand("/send_cam_to_cb", pszBuff))
		{
		...
		else if (ChatCommand("/send_xyz_to_cb", pszBuff))
		{
			Point3D Coordinate = UNITDATA->sPosition;
		...
		else if (ChatCommand("/stream_mode_off", pszBuff))
		{
		...
		else if (ChatCommand("/st", pszBuff) || ChatCommand("/set_time", pszBuff))
		{
			GetParameterString(pszBuff, 1, szChatCommandParam1);
		...
		else if (ChatCommand("/set_dark_level", pszBuff))
		{
			GetParameterString(pszBuff, 1, szChatCommandParam1);
			GetParameterString(pszBuff, 2, szChatCommandParam2);
		...
		else if (ChatCommand("/add_text_chat", pszBuff))
		{
			GetParameterString(pszBuff, 1, szChatCommandParam1);
			GetParameterString(pszBuff, 2, szChatCommandParam2);
		...
		else if (ChatCommand("/add_title_box", pszBuff))
		{
			GetParameterString(pszBuff, 1, szChatCommandParam1);
			GetParameterString(pszBuff, 2, szChatCommandParam2);
		...
		else if (ChatCommand("/add_message_box", pszBuff))
		{
			GetParameterString(pszBuff, 1, szChatCommandParam1);
			GetParameterString(pszBuff, 2, szChatCommandParam2);
		...
		else if (ChatCommand("/affinity_p", pszBuff))
		{
			GetParameterString(pszBuff, 1, szChatCommandParam1);
		...
		else if (ChatCommand("/reload_scripts", pszBuff))
		{
			CALL(0x00574740);
			AddChatBoxText("> Scripts reloaded!");
		}
		else if (ChatCommand("/reset_force", pszBuff))
		{
		...
		else if (ChatCommand("/anim_tt", pszBuff))
		{
			GetParameterString(pszBuff, 1, szChatCommandParam1);
		...
		else if (ChatCommand("/test_w", pszBuff))
		{

		}
		else if (ChatCommand("/set_integrity", pszBuff))
		{
			GetParameterString(pszBuff, 1, szChatCommandParam1);
		...
		else if (ChatCommand("/set_view_sight_sql", pszBuff))
		{
			GetParameterString(pszBuff, 1, szChatCommandParam1);
		...
		else if (ChatCommand("/sql_skill_min_max", pszBuff) ||
			ChatCommand("/sql_attack_min_max", pszBuff))
		{
		...
		//sql command that requires 1 parameter
			...
			ChatCommand("/sql_stun_chance", pszBuff) || //Stun chance (doesn't work?)
			ChatCommand("/sql_defense", pszBuff) || //Defense
			ChatCommand("/sql_potion", pszBuff) || //Potion
			ChatCommand("/sql_potion_percent", pszBuff) || //Potion percentage
			...
			ChatCommand("/sql_spawn_min", pszBuff) || //Spawn min
			ChatCommand("/sql_spawn_max", pszBuff) || //Spawn max
			ChatCommand("/sql_move_speed", pszBuff) || //Move speed (doesn't work?)
			ChatCommand("/sql_attack_speed", pszBuff) || //Attack speed (doesn't work?)
			ChatCommand("/sql_attack_range", pszBuff) || //Attack range
			ChatCommand("/sql_attack_rating", pszBuff) || //Attack rating
			ChatCommand("/sql_perfect_attack_rate", pszBuff) || //Perfect attack rating (doesn't work?)
			ChatCommand("/sql_skill_type", pszBuff) || //Skill type
			ChatCommand("/sql_skill_chance", pszBuff) || //Skill chance
			ChatCommand("/sql_view_sight", pszBuff) || //View sight
			ChatCommand("/sql_num_drops", pszBuff) || //Number of loot drops
			ChatCommand("/sql_skill_hit_range", pszBuff) ||	//iSkillPierceRange // SkillDistance (uses a rect box to determine hit area?)
			ChatCommand("/sql_skill_hit_box_left", pszBuff) ||	//Skill hit box rect (left)
			ChatCommand("/sql_skill_hit_box_right", pszBuff) ||	//Skill hit box rect (right)
			ChatCommand("/sql_skill_hit_box_top", pszBuff) ||	//Skill hit box rect (top)
			ChatCommand("/sql_skill_hit_box_bottom", pszBuff) ||	//Skill hit box rect (bottom)
			ChatCommand("/sql_glow", pszBuff) ||	//Glow Bossflag
			ChatCommand("/sql_skill_area", pszBuff) ||	//iSkillArea // SkillRange
		...
		else if (ChatCommand("/leak_monster_test", pszBuff))
		{
			if (CURRENT_TARGET != 0)
				this->SendChat(FALSE, FormatString("/LeakMonsterTest %d", CURRENT_TARGET->iID));
		}
		else if (ChatCommand("/leak_packet", pszBuff))
		{
		...
		else if (ChatCommand("/open_window_test", pszBuff))
		{
			GetParameterString(pszBuff, 1, szChatCommandParam1);
			GRANDFURYHANDLER->GetWindow()->Open(atoi(szChatCommandParam1));
		}
		else if (ChatCommand("/close_window_test", pszBuff))
		{
			GRANDFURYHANDLER->GetWindow()->Close();
		}
		else if (ChatCommand("/test_pp", pszBuff))
		{
		...
		else if (ChatCommand("/train_skills", pszBuff))
		{
		...
		else if (ChatCommand("/test_rankup", pszBuff))
		{
#ifdef DEV_MODE
			GetParameterString(pszBuff, 1, szChatCommandParam1);
		...
		else if (ChatCommand("/skip_tier", pszBuff))
		{
			GetParameterString(pszBuff, 1, szChatCommandParam1);
		...
		else if (ChatCommand("/delete_item_test", pszBuff))
		{
			if (GetParameterString(pszBuff, 1, szChatCommandParam1))
			{
		...
		else if (ChatCommand("/skip_quests", pszBuff))
		{
			CQUESTGAME->SkipOriginalQuests(TRUE);
		}
		else if (ChatCommand("/skip_quest_30", pszBuff))
		{
		...
		else if (ChatCommand("/skip_quest_55", pszBuff))
		{
		...
		else if (ChatCommand("/skip_quest_70", pszBuff))
		{
		...
		else if (ChatCommand("/skip_quest_80a", pszBuff))
		{
		...
		else if (ChatCommand("/skip_quest_80b", pszBuff))
		{
		...
		else if (ChatCommand("/skip_quest_85", pszBuff))
		{
		...
		else if (ChatCommand("/skip_quest_90a", pszBuff))
		{
		...
		else if (ChatCommand("/skip_quest_90b", pszBuff))
		{
