...
BOOL  AccountServer::GetSQLUserInfoData(const char* pszAccountName, SQLUser* psSqlUserOut)
{
	...
	{
		WARN("GetSQLUserInfoData: Invalid account name parameter");
		return FALSE;
	}

	// Log the exact account name being searched
	WARN("GetSQLUserInfoData: Attempting to query account '%s' (length: %d)", pszAccountName, strlen(pszAccountName));
	...
	if (cDB->Open())
	{
		if (cDB->Prepare("SELECT TOP(1) [ID],[AccountName],[Password],[Flag],[Active],[Coins],[GameMasterType],[GameMasterLevel],[GameMasterMacAddress],[BanStatus],[UnbanDate],[IsMuted],[MuteCount],[UnmuteDate] FROM UserInfo WHERE AccountName=?"))
		{
			...
			// Check if Execute succeeds first
			if (cDB->Execute())
			{
				...
				if (cDB->Fetch())
				{
					...
					cDB->GetData(3, PARAMTYPE_String, psSqlUserOut->szPassword, 65);
					...
					cDB->GetData(12, PARAMTYPE_Integer, &psSqlUserOut->bIsMuted);
					cDB->GetData(13, PARAMTYPE_Integer, &psSqlUserOut->iMuteCount);
					cDB->GetData(14, PARAMTYPE_Time, &psSqlUserOut->sUnmuteDate);

					psSqlUserOut->iGameMasterLevel = (EGameLevel)iGameMasterLevel;
					psSqlUserOut->iBanStatus = (EBanStatus)iBanStatus;

					bResult = TRUE;
					WARN("GetSQLUserInfoData: Successfully retrieved data for account '%s'", pszAccountName);
				}
				else
				{
					WARN("GetSQLUserInfoData: Fetch failed - No rows returned for account '%s'", pszAccountName);
				}
			}
			else
			{
				WARN("GetSQLUserInfoData: Execute failed for account '%s' - Check database connection and query", pszAccountName);
			}
		}
		else
		{
			WARN("GetSQLUserInfoData: Prepare failed for account '%s' - Check SQL syntax and table structure", pszAccountName);
		}
	...
	{
		WARN("GetSQLUserInfoData: Failed to open database connection for account '%s'", pszAccountName);
	}
